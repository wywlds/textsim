...
class Model(object):
    ...
    def __init__(self, sess, is_Training=True, batch_size=25):
        ...
        with tf.name_scope('Vector_Layer'):
            self.sent1 = tf.reduce_sum(self.cell_outputs1 * self.mask_s1[:, :, None], axis=1) / tf.reduce_sum(self.mask_s1[:,:,None], axis=1)
            self.sent2 = tf.reduce_sum(self.cell_outputs2 * self.mask_s2[:, :, None], axis=1) / tf.reduce_sum(self.mask_s2[:,:,None], axis=1)
